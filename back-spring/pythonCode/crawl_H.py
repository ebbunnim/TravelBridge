...
def insertToDB(hp):
    ...
    try:
        with db.cursor() as cursor:  # control structure of database
            fakeStr = str(hp[hp_key[0]]) +',"'+ str(hp[hp_key[1]]) +'","'+ str(hp[hp_key[2]])+'","'+ str(hp[hp_key[3]]) +'","'+ str(hp[hp_key[4]]) +'","'+ str(hp[hp_key[5]]) +'","'+ str(hp[hp_key[6]]) +'","'+ str(hp[hp_key[7]])+'","'+ str(hp[hp_key[8]])+'","'+ str(hp[hp_key[9]])+'","'+ str(hp[hp_key[10]]+'"')
            sql = """
                INSERT INTO 
                HOTPLACE
...
for i in range(43, 3000):
    ...
    #img
    try:
        elem = find_xpath('//*[@id="contents"]/div[2]/div[2]/div[1]/div[1]/div/div/span/img').get_attribute("src")
        hp[hp_key[9]] = elem
    except:
        hp[hp_key[9]] = " "
        pass
